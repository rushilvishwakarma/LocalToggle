...
import { Clock } from "lucide-react";  // Changed to Clock icon
...
function convertTime(value: number, fromUnit: string, toUnit: string): number {
  ...
  return seconds / toSeconds[toUnit];
}
...
export function TimeConverter() {
  ...
  useEffect(() => {
    ...
    if (!isNaN(numericValue)) {
      const result = convertTime(numericValue, fromUnit, toUnit);
        ...
        <div className="py-3 px-3">
          <div className="flex flex-col gap-1 text-left">
            ...
            <div className="flex items-center justify-center bg-white bg-opacity-[0.05] w-16 h-16 rounded-full">
              <Clock className="text-gray-400 w-7 h-7 strokeWidth={1}" />
            </div>
          </div>
        </div>
